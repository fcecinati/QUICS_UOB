...
saveas(error_corr,'error_correlation','fig')
hold off
close(error_corr)
...
% 
% 
% % Noooooooooo.... problem with nan and consecutive values!
% time_corr = zeros(x,3);
% for j=1:x
%     vector=err(:,j);
%     vector(isnan(vector))=[];
%     time_corr(j,:)=autocorr(vector,2);
% end
...
figure(error_corr_d)
scatter(d,c)
title('spatial correlation of the generated errors');
xlabel('meters')
hold on
% plot(plx,ply1,'r')
plot(plx,ply2,'g')
legend('generated error correlation coefficients',str2)
saveas(error_corr_d,'error_correlation_d','fig')
hold off
close(error_corr)
