...
if plot_spa == 'Y'
    ...
    saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\error_spatial_correlation','fig')
    saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\error_spatial_correlation','jpg')
    close(fig)
end
...
if plot_tem == 'Y'
    ...
    xlabel('hours')
    saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\error_temporal_correlation','fig')
    saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\error_temporal_correlation','jpg')
    close(fig)
end
...
if cov_dec == 'Y'
    ...
    saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\covariance_test','fig')
    saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\covariance_test','jpg')
    close(fig)
end
...
if spa_dec == 'Y'
    for ens=1:n
        ...
        figure(fig)
        plot(myfit, d, c)
        title(['spatial correlation of the generated errors n.' num2str(f)]);
        xlabel('meters x 10^5')
        name = ['\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\error_correlation_delta_' num2str(f)];
        saveas(fig,name,'fig')
        saveas(fig,name,'jpg')
        close(fig)
    end
...
if tem_dec == 'Y'
    for ens=1:n
        ...
        for lag=1:3
            for j=1:x
                v1 = delta(1:(t-lag-2),j);
                v2 = delta((1+lag):(t-2),j);
                m1 = nanmean(v1);
                m2 = nanmean(v2);
                sd1 = nanstd(v1);
                sd2 = nanstd(v2);
        ...
        xlabel('hours')
        saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\delta_temporal_correlation','fig')
        saveas(fig,'\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\delta_temporal_correlation','jpg')
        close(fig)
    end
end

%% saving the results
save ('\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Germann_no_weightings\delta.mat','delta')

%% Generate the ensembles
ensembles = Ensembles(delta, coord, sim_hours);
