...
% Results path
results_path = '\\ads.bris.ac.uk\filestore\MyFiles\Staff3\fc14509\Documents\QUICS\MATLAB_rep\Results\Test_German\Test_8\';
% 
    ...
    for ens=1:n
        ...
        for lag=1:2
            for j=1:x
                ...
                sd1 = nanstd(v1);
                sd2 = nanstd(v2);
                rho_d(j,1+lag)=(nanmean((v1-m1).*(v2-m2)))/(sd1*sd2);
            end
        end
        maxrhod = max(rho_d);
        minrhod = min(rho_d);
        ...
        plot(x_axis, minrho, 'g')
        plot(x_axis,rhod,'k')
        plot(x_axis, maxrhod, 'c')
        plot(x_axis, minrhod, 'c')
        title('temporal correlation of the errors');
        xlabel('hours')
        legend ('original','original max','original min','delta','delta max','delta min')
